...
def tfce_transform(img, H, E, dh):
    ...
    for threshold in threshs:
        ...
        comp, _ = ndimage.label(
            (img > threshold).astype('int'), structure=struct)
        ...
        # Build an image where each voxel contains the size of its connected
        # component.
...
def onesample_perm(imgs, implicitmask, tvals, subjects, root_subjects, bsize,
                   H, E, dh):
    """One sample permutation test."""
    roccimgs = np.copy(imgs)
    np.random.seed()
    # Permuate flipping signs at random.
    for subject in [subject for subject in np.arange(subjects)
                    if np.random.choice([True, False])]:
        roccimgs[:, subject] = -imgs[:, subject]

    # Calculate the permutation statistic.
    rstats = np.mean(roccimgs, axis=1) / \
        (np.std(roccimgs, axis=1) / root_subjects)
...
def tfce_onesample(imgs, nperm=5000, H=2, E=0.5, dh=0.1):
    """Test one sample hypothesis mean > 0.
    ...
    -----------------
     ...
     Returns
     -------
        Numpy array of shape x * y * z * n_images containing corrected
        p-values.
    ...
    truestat = np.nanmean(imgs, axis=3) / \
        (np.nanstd(imgs, axis=3) / root_subjects)
    ...
    tfce_pool = Pool()
    permdata = itertools.repeat((occimgs, implicitmask, tvals, subjects,
                                 root_subjects, bsize, H, E, dh), nperm)
